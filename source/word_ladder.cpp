...
namespace word_ladder {
	...
	[[nodiscard]] auto generate(std::string const& from,
	   ...
	   -> std::vector<std::vector<std::string>> {
		...
		auto const same_length = [word_length](std::string const& lex) {
			return lex.length() == word_length;
		};

		auto filtered_lexicon = lexicon | ranges::views::filter(same_length)
			...
			while (!ladder_queue.empty() and ladder_queue.front().size() <= curr_size) {
				...
				for (auto pos = static_cast<unsigned long>(0); pos < word_length; pos++) {
					...
					for (auto c = 'a'; c <= 'z'; c++) {
						...
						// If NOT observed, add it to the map. Else check if its in the same level. Else
						// reject.
